...
class Cyclone():
  ...
  def mkInstDictC_objTC(self, iYM, eYM, varname="vortlw"):
    ...
    lvar      = ["dura","nowpos","time","vortlw","dtlow","dtmid","dtup","initsst","initland","wmeanup","wmeanlow","dtsum"]  # wmean added.
    ...
    for Year,Mon in lYM:
      for var in lvar:
        if var=='dtsum':
          da1[var]  = self.load_clist('dtlow', Year, Mon)\
                     +self.load_clist('dtmid', Year, Mon)\
